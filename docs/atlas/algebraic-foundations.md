# Algebraic Foundations: The Tensor Product Structure

This document presents the algebraic substrate of Atlas: **SGA = Cl₀,₇ ⊗ ℝ[ℤ₄] ⊗ ℝ[ℤ₃]**

This is not a "design" but a **discovered mathematical structure** - the minimal algebraic entity supporting geometric computation with discrete symmetries.

## The Three Factors

Atlas emerges from the tensor product of three fundamental algebras. Each factor is unique and inevitable.

### Factor 1: Clifford Algebra Cl₀,₇

**Definition**: The Clifford algebra generated by 7 orthonormal basis vectors {e₁, e₂, ..., e₇} with Euclidean signature.

**Geometric Product**:
```
eᵢ · eⱼ = eᵢ ∧ eⱼ  (i ≠ j)
eᵢ · eᵢ = 1         (Euclidean)
```

Equivalently, the anticommutation relation:
```
eᵢeⱼ + eⱼeᵢ = 2δᵢⱼ
```

**Basis Structure**:
- **Grade 0** (scalar): 1 element = (₇C₀) = 1
- **Grade 1** (vectors): 7 elements = (₇C₁) = 7
- **Grade 2** (bivectors): 21 elements = (₇C₂) = 21
- **Grade 3** (trivectors): 35 elements = (₇C₃) = 35
- **Grade 4**: 35 elements = (₇C₄) = 35
- **Grade 5**: 21 elements = (₇C₅) = 21
- **Grade 6**: 7 elements = (₇C₆) = 7
- **Grade 7** (pseudoscalar): 1 element = (₇C₇) = 1

**Total dimension**: 2⁷ = 128

**Implementation**: [sga/clifford.ts](../../packages/core/src/sga/clifford.ts)

**Why 7 dimensions?**

The dimension 7 is tied to **octonions** - the unique 8-dimensional normed division algebra:
```
𝕆 = ℝ ⊕ Im(𝕆)
   = scalar ⊕ 7 imaginary units
```

The 7 imaginary units {e₁, ..., e₇} multiply according to the **Fano plane** (see [Geometric Foundations](./geometric-foundations.md)).

**Key Property**: Cl₀,₇ is the **maximal Clifford algebra** that remains computationally tractable while preserving rich geometric structure. Dimension 8 would require 256 basis blades; dimension 7 requires 128 (still manageable).

**Involutions**:
```
Grade involution:    [eᵢ₁...eᵢₖ]ˆ = (-1)^k eᵢ₁...eᵢₖ
Reversion:           [eᵢ₁...eᵢₖ]~ = eᵢₖ...eᵢ₁
Clifford conjugate:  [x]* = [[x]~]ˆ
```

These involutions are used in the M (mirror) transform.

### Factor 2: Group Algebra ℝ[ℤ₄]

**Definition**: The real group algebra of the cyclic group of order 4.

**Basis**: {r⁰, r¹, r², r³} where r is the generator.

**General element**:
```
a = a₀r⁰ + a₁r¹ + a₂r² + a₃r³    (aᵢ ∈ ℝ)
```

**Multiplication**: Convolution
```
(Σ aᵢrⁱ)(Σ bⱼrʲ) = Σₖ (Σᵢ₊ⱼ≡ₖ aᵢbⱼ) rᵏ
```

where indices are reduced mod 4.

**Implementation**: [sga/group-algebras.ts](../../packages/core/src/sga/group-algebras.ts) (z4Multiply)

**Matrix Representation**: Multiplication by r is a circulant matrix:
```
r · [a₀, a₁, a₂, a₃]ᵀ = [a₃, a₀, a₁, a₂]ᵀ
```

**Circulant matrix** for general element a = [a₀, a₁, a₂, a₃]:
```
     [a₀  a₃  a₂  a₁]
M =  [a₁  a₀  a₃  a₂]
     [a₂  a₁  a₀  a₃]
     [a₃  a₂  a₁  a₀]
```

**Key Properties**:
- **Commutative**: ab = ba
- **Identity**: r⁴ = 1
- **Inversion**: (rᵏ)⁻¹ = r⁻ᵏ = r⁽⁴⁻ᵏ⁾

**Semantic Interpretation**:
- r⁰, r¹, r², r³ correspond to **quadrants** or **scopes**
- Encoded in (h₂, d, ℓ) coordinate as **h₂ ∈ {0,1,2,3}**

**Why ℤ₄?**

Four-fold rotational symmetry is the **minimal structure** supporting:
1. Cardinal directionality (4 orthogonal directions)
2. Quaternionic phase (i² = -1 analogy in ℤ₄: r² is self-inverse)
3. Complete rotational coverage (360° in 4 steps)

No smaller cyclic group provides these properties. Larger groups are redundant.

### Factor 3: Group Algebra ℝ[ℤ₃]

**Definition**: The real group algebra of the cyclic group of order 3.

**Basis**: {τ⁰, τ¹, τ²} where τ is the generator.

**General element**:
```
b = b₀τ⁰ + b₁τ¹ + b₂τ²    (bᵢ ∈ ℝ)
```

**Multiplication**: Convolution (mod 3)
```
(Σ bᵢτⁱ)(Σ cⱼτʲ) = Σₖ (Σᵢ₊ⱼ≡ₖ bᵢcⱼ) τᵏ
```

**Implementation**: [sga/group-algebras.ts](../../packages/core/src/sga/group-algebras.ts) (z3Multiply)

**Matrix Representation**: Multiplication by τ is a circulant matrix:
```
     [b₀  b₂  b₁]
M =  [b₁  b₀  b₂]
     [b₂  b₁  b₀]
```

**Key Properties**:
- **Commutative**: bc = cb
- **Identity**: τ³ = 1
- **Inversion**: (τᵏ)⁻¹ = τ⁻ᵏ = τ⁽³⁻ᵏ⁾

**Semantic Interpretation**:
- τ⁰ = **neutral** modality
- τ¹ = **produce** modality
- τ² = **consume** modality
- Encoded in (h₂, d, ℓ) coordinate as **d ∈ {0,1,2}**

**Why ℤ₃?**

Triadic structure is the **minimal extension beyond binary**:
- Binary (ℤ₂): Producer/consumer only, no neutral
- Triadic (ℤ₃): Neutral + producer + consumer (minimal complete system)
- Higher (ℤ₄, ℤ₅, ...): Redundant subdivisions

**Triality** is fundamental: every producer has a corresponding consumer, with neutral as the mediating ground.

## The Tensor Product: SGA

**Definition**:
```
SGA := Cl₀,₇ ⊗_ℝ ℝ[ℤ₄] ⊗_ℝ ℝ[ℤ₃]
```

**Dimension**:
```
dim(SGA) = dim(Cl₀,₇) × dim(ℝ[ℤ₄]) × dim(ℝ[ℤ₃])
         = 128 × 4 × 3
         = 1,536
```

**Basis**:
```
B = {eᵢ₁∧...∧eᵢₖ ⊗ rʰ ⊗ τᵈ : k ∈ {0..7}, h ∈ {0..3}, d ∈ {0..2}}
```

**General element**:
```
x = Σ cₖ,ₕ,ᵈ (eᵢ₁∧...∧eᵢₖ ⊗ rʰ ⊗ τᵈ)
```

**Multiplication**: Componentwise geometric product × convolution × convolution
```
(a ⊗ rʰ ⊗ τᵈ) · (b ⊗ rᵍ ⊗ τᵉ) = (a·b) ⊗ rʰ⁺ᵍ ⊗ τᵈ⁺ᵉ
```

where:
- a·b is the Clifford geometric product
- h+g is computed mod 4
- d+e is computed mod 3

**Implementation**: [sga/](../../packages/core/src/sga/)

## Rank-1 Restriction: The 96-Class Basis

**Problem**: Full SGA has 1,536 dimensions - computationally intractable for symbolic manipulation.

**Solution**: Restrict to **rank-1 elements** in the Clifford component.

**Rank-1 basis**:
```
E_{h,d,ℓ} = r^h ⊗ e_ℓ ⊗ τ^d
```

where:
- h ∈ {0,1,2,3} - quadrant index (from ℤ₄)
- ℓ ∈ {0,1,2,3,4,5,6,7} - context index (from Cl₀,₇)
  - ℓ = 0: scalar (grade 0)
  - ℓ = 1..7: basis vectors e₁, ..., e₇ (grade 1)
- d ∈ {0,1,2} - modality index (from ℤ₃)

**Count**:
```
|{E_{h,d,ℓ}}| = 4 × 8 × 3 = 96
```

**This is where the 96 classes emerge.**

**Key Insight**: Rank-1 elements form a **spanning set** for computational purposes:
- Linear combinations of rank-1 elements can approximate higher-rank elements
- 96 classes provide a **discrete coordinate system** for the continuous SGA space
- Computationally tractable (finite) while algebraically rich (embedded in SGA)

**Implementation**:
- [class-system/class.ts](../../packages/core/src/class-system/class.ts): Class index ↔ (h₂, d, ℓ) bijection
- [bridge/lift.ts](../../packages/core/src/bridge/lift.ts): Class → SGA embedding

## Coordinate Encoding

**Class index formula**:
```
class(h, d, ℓ) = 24h + 8d + ℓ
```

**Inversion**:
```
h = ⌊class / 24⌋
d = ⌊(class mod 24) / 8⌋
ℓ = class mod 8
```

**Byte encoding**:
```
h₂ = (b₇ << 1) | b₆          (bits 7-6 encode quadrant)
d  = decode(b₅, b₄)          (bits 5-4 encode modality)
ℓ  = (b₃ << 2) | (b₂ << 1) | b₁  (bits 3-1 encode context)
b₀ = 0                        (LSB cleared for canonical form)
```

**Modality encoding**:
```
d=0 (neutral):  b₅=0, b₄=0
d=1 (produce):  b₅=0, b₄=1
d=2 (consume):  b₅=1, b₄=0
(b₅=1, b₄=1 is unused - creates equivalence classes)
```

**Implementation**: [class-system/class.ts](../../packages/core/src/class-system/class.ts) (encodeComponentsToByte)

## Algebra Operations on Rank-1 Elements

While rank-1 elements are restricted to grade ≤ 1 in Clifford component, they still support:

**Addition** (inherits from SGA):
```
E_{h,d,ℓ} + E_{h',d',ℓ'} ∈ SGA  (may leave rank-1 subspace)
```

**Scalar multiplication** (stays in rank-1):
```
α · E_{h,d,ℓ} = E_{h,d,ℓ}  (up to normalization)
```

**Geometric product** (may leave rank-1):
```
E_{h,d,ℓ} · E_{h',d',ℓ'} ∈ SGA  (possibly higher grade)
```

**Automorphisms** (stay in rank-1):
```
R(E_{h,d,ℓ}) = E_{(h+1 mod 4),d,ℓ}
D(E_{h,d,ℓ}) = E_{h,(d+1 mod 3),ℓ}
T(E_{h,d,ℓ}) = E_{h,d,(ℓ+1 mod 8)}
M(E_{h,d,ℓ}) = E_{h,(3-d mod 3),ℓ}
```

**Key Property**: Transforms preserve rank-1 structure - critical for computational tractability.

**Implementation**:
- [sga/transforms.ts](../../packages/core/src/sga/transforms.ts): Full SGA transforms
- [class-system/class.ts](../../packages/core/src/class-system/class.ts): Class permutations

## The Bridge: Class ↔ SGA

The **bridge** is a pair of functorial mappings:

**Lift**: Class Index → SGA Element
```
lift: {0..95} → SGA
lift(c) = r^h ⊗ e_ℓ ⊗ τ^d    where (h,d,ℓ) = decompose(c)
```

**Project**: SGA Element → Class Index
```
project: SGA → {0..95} ∪ {error}
project(r^h ⊗ e_ℓ ⊗ τ^d) = class(h,d,ℓ)
project(x) = error  if x is not rank-1
```

**Fundamental Property (Commutative Diagrams)**:

For all transforms g ∈ {R, D, T, M} and classes c ∈ {0..95}:
```
project(g_SGA(lift(c))) = g_class(c)
```

**Verification**: [bridge/validation.ts](../../packages/core/src/bridge/validation.ts)
- 1,248 commutative diagram tests (all pass)
- Proves that class permutations and SGA automorphisms are "the same thing"

This is **not** empirical testing - it is **exhaustive verification** for the finite system.

## Why This Algebraic Structure?

**Inevitability argument**:

1. **Need geometric structure**: Clifford algebra (most general geometric product)
2. **Need discrete symmetries**: Group algebras ℤ₄ (quadrants) and ℤ₃ (triality)
3. **Need computational tractability**: Rank-1 restriction (96 classes)
4. **Need correctness**: Bridge verification (algebra = permutations)

**Each choice is forced by universal properties** (see [Universal Properties](./universal-properties.md)).

The algebraic foundation is **not designed** - it is the **unique minimal structure** satisfying these requirements.

## Summary Table

| Algebra Factor | Dimension | Generator | Semantic Role | Implementation |
|----------------|-----------|-----------|---------------|----------------|
| Cl₀,₇ | 128 | e₁..e₇ | Context (what) | [sga/clifford.ts](../../packages/core/src/sga/clifford.ts) |
| ℝ[ℤ₄] | 4 | r | Scope (where) | [sga/group-algebras.ts](../../packages/core/src/sga/group-algebras.ts) |
| ℝ[ℤ₃] | 3 | τ | Modality (how) | [sga/group-algebras.ts](../../packages/core/src/sga/group-algebras.ts) |
| **SGA (full)** | **1,536** | — | Full algebra | [sga/](../../packages/core/src/sga/) |
| **Rank-1 Basis** | **96** | — | Computational substrate | [class-system/](../../packages/core/src/class-system/) |

---

**Conclusion**: The algebraic foundation of Atlas is a **discovered structure** - the tensor product **Cl₀,₇ ⊗ ℝ[ℤ₄] ⊗ ℝ[ℤ₃]** restricted to rank-1 elements. The 96 classes are not arbitrary but **inevitable**.
