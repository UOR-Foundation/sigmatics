...
function isPrime96(n: number): boolean {
...
function isPrimePower96(n: number): boolean {
  const factors = factor96Model.run({ n });
...
console.log(
  `Prime power count: ${primePowerCount}/${orbit37.length} (${((primePowerCount / orbit37.length) * 100).toFixed(1)}%)\n`,
);
...
for (const { k, value, factors, prime, primePower } of e7Multiples) {
  ...
  if (k <= 20 || k % 10 === 0) {
    console.log(`│ ${k.toString().padStart(2)} │ ${value.toString().padStart(4)} │ ${factorStr} │ ${typeStr.padEnd(11)} │`);
  }
}
...
const orbitDiameter = Math.max(...distance.values());
console.log(`Orbit diameter: ${orbitDiameter}`);
console.log(`E₇ dimension: 133`);
console.log(`133 / ${orbitDiameter} = ${(133 / orbitDiameter).toFixed(3)}`);
console.log(`133 = ${Math.floor(133 / orbitDiameter)} × ${orbitDiameter} + ${133 % orbitDiameter}`);
console.log(`\nE₇ dimension is exactly ${Math.floor(133 / orbitDiameter)} full orbit cycles + ${133 % orbitDiameter}!\n`);
...
console.log(`   - Diameter: ${orbitDiameter}`);
console.log(`   - E₇ dimension: 133 = ${Math.floor(133 / orbitDiameter)} × ${orbitDiameter} + ${133 % orbitDiameter}`);
